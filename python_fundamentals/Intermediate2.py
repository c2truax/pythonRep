...
def iterateDictionarytwo(keyName, dicty):
    for count in range(len(dicty)):
        print(dicty[count][keyName])
...
def printDojoInfo(dicty):
    for key, value in dicty.items():
        print(len(dicty[key]), " ", key)
        for count in range(len(dicty[key])):
            print(dicty[key][count])